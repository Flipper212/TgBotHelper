...
monday = [None, "Фізкультура", "Географія", "Геометрія", "Геометрія", "Фізика", "Історія", "Укр літ", "СК"]
tuesday = [None, "Алгебра", "Алгебра", "Історія", "Трудове", "Англ/Укр мова", "Фізика", "Заруб літ", "Інформатика/Англ мова"]
wednesday = [None, "Алгебра", "Алгебра", "Укр мова/Інформ", "Укр літ", "Інформ/Укр мова", "Фізкультура", "Фізика"]
thursday = [None, "Мистецтво", "ОЗ", "Геометрія", "Укр мова/Інформ", "Англ мова", "Хімія", "Біологія"]
friday = [None, "Право", "Алгебра", "Хімія", "Географія", "Біологія", "Історія", "Фізкультура"]
...
def main():
    ...
    global is_start, is_end, is_work_day, result_list
    ...
    if 0 < count_day < 6:
        ...
        if time_str < 830:
            ...
            is_start = False
        elif 830 <= time_str < 1430 and 2 < count_day < 6:
            while int(list(lesson_time_int.values())[index]) < time_str:
                index += 1
        elif 830 <= time_str < 1520 and 0 < count_day < 3:
            while int(list(lesson_time_int.values())[index]) < time_str:
                index += 1
